...
/* ---- Audio Manager (unique, défini avant toute scène) ---- */
class AudioMgr{
...
class IntroScene extends Phaser.Scene{
  ...
  _logo(Ltxt,Rtxt,Lcol,Rcol,done){
    ...
    if(getSettings().sfx) this.sound.play('open',{volume:0.8});
...
class MenuScene extends Phaser.Scene{
  ...
  create(){
    ...
    let y=250;
    items.forEach(e=>{
      const t=this.add.text(400,y,e.label,{fontFamily:'"Press Start 2P"',fontSize:'16px',color:e.disabled?COLORS.grey:COLORS.white}).setOrigin(0.5);
      y+=50;
      if(!e.disabled){
        t.setInteractive({useHandCursor:true});
        t.on('pointerover',()=>{if(getSettings().sfx)this.sound.play('hover',{volume:0.7}); t.setColor(COLORS.gold);
        ...
        t.on('pointerdown',()=>{if(getSettings().sfx)this.sound.play('click'); this._select(e.id);});
      }
    });
  ...
  _select(id){
    if(id==='new'){
      ...
      if(getSettings().sfx) this.sound.play('start'); this.scene.start('BootScene');
    }
    if(id==='continue' && hasSave()){ if(getSettings().sfx)this.sound.play('continue'); this.scene.start('BootScene'); }
    if(id==='options') this._toggleOptions(true);
    if(id==='credits') this._toggleCredits(true);
  }

  _warnNewGame(){
    if(getSettings().sfx) this.sound.play('open');
    ...
    document.getElementById('w-cancel').onclick=()=>{ if(getSettings().sfx)this.sound.play('back'); document.body.removeChild(overlay); };
    document.getElementById('w-ok').onclick=()=>{ if(getSettings().sfx)this.sound.play('click'); resetSave(); document.body.removeChild(overlay); this.scene.start('BootScene'); };
  }

  /* --- Options --- */
  _buildOptions(){
    const cx=400,cy=300,s=getSettings(), p=(k)=>{ if(getSettings().sfx)this.sound.play(k,{volume:0.8}); };
...
/* 4) Boot du niveau (précharge spécifique & transition) */
class BootScene extends Phaser.Scene{
  constructor(){ super('BootScene'); }
  preload(){
    const t=this.add.text(400,560,'Chargement',{fontFamily:'"Press Start 2P"',fontSize:'12px',color:'#fff'}).setOrigin(0.5,1);
    let d=0; this.time.addEvent({delay:350,loop:true,callback:()=>{d=(d+1)%4; t.setText('Chargement'+'.'.repeat(d));}});
    // (On réutilise les assets déjà chargés en PreloadScene — ici, rien d’obligatoire)
...
class DifficultyScene extends Phaser.Scene{
  ...
  create(){
    ...
    const opts=['Facile','Normal','Difficile']; let x=400-150;
    opts.forEach(label=>{
    ...
    this.add.text(400,360,'Appuie pour lancer le niveau 1-0',{fontFamily:'"Press Start 2P"',fontSize:'10px',color:'#ccc'}).setOrigin(0.5);
  }
}
...
class GameIntroScene extends Phaser.Scene{
  ...
  create(){
    ...
    // Fond & nuages dessinés en code (pas d’images nécessaires)
    ...
    this.time.delayedCall(10000,()=>{ if(this.state==='intro_fall'){ this.player.setVelocityY(0); this.player.y = -30; }});
  }
