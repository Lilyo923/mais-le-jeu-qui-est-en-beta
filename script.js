...
class PreloadScene extends Phaser.Scene{
  ...
  preload(){
    ...
    const s=['click','open','poweroff','start','continue','back'];
    s.forEach(v=>this.load.audio(v,`assets/sounds/${v}.wav`));
...
class IntroScene extends Phaser.Scene{
  ...
  _playLogo(l,r,lc,rc,s,onDone){
    ...
    this.time.delayedCall(2300,()=>{
      ...
      this.tweens.add({targets:[L,R],scaleY:0.05,alpha:0.8,duration:500,ease:'quad.in',onComplete:()=>{L.destroy();R.destroy();this.time.delayedCall(400,onDone);}});
    });
  }
}
...
class MenuScene extends Phaser.Scene{
  ...
  create(){
    ...
    const arrow=this.add.text(0,0,'▶',{fontFamily:'"Press Start 2P"',fontSize:'14px',color:'#fff'}).setVisible(false);
    let y=250;
    data.forEach((e,i)=>{
      const col=e.disabled?COLORS.grey:COLORS.white;
      const t=this.add.text(400,y+i*50,e.label,{fontFamily:'"Press Start 2P"',fontSize:'16px',color:col}).setOrigin(0.5);
      if(!e.disabled){
        t.setInteractive({useHandCursor:true});
        t.on('pointerover',()=>{t.setColor(COLORS.gold);arrow.setPosition(t.x-t.width/2-22,t.y).setVisible(true);});
  ...
  _onSelect(id){
    ...
    if(id==='new'){play('start');localStorage.removeItem(SAVE_KEY);alert('Nouvelle partie (niveau 1-0)');}
    if(id==='continue'&&localStorage.getItem(SAVE_KEY)){play('continue');alert('Chargement partie…');}
  ...
  _buildOptions(){
    ...
    const L=this.add.text(cx+30,cy-45,'<',{fontFamily:'"Press Start 2P"',fontSize:'12px',color:'#fff'}).setOrigin(0.5).setInteractive().setVisible(false);
    const R=this.add.text(cx+90,cy-45,'>',{fontFamily:'"Press Start 2P"',fontSize:'12px',color:'#fff'}).setOrigin(0.5).setInteractive().setVisible(false);
